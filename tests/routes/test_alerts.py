...
def _client() -> tuple[TestClient, ConsoleNotifier]:
    ...
    app = create_app(db_path=Path(tempfile.mkdtemp()) / "a.db", clock=FixedClock(NOW),
                     notifier=notifier, config_path=ROOT / "rules_config.toml",
                     reference=(None, "task R2 outstanding"), start_scheduler=False)
...
def test_red_finding_is_pinned_then_acknowledged() -> None:
    ...
    r = client.post("/api/alerts/acknowledge",
...
def test_amber_findings_shown_without_acknowledge_button() -> None:
    ...
    client.post("/api/adjust-time",
                data={"table": "feed", "event_id": 1, "ts": old})
    client.app.state.services.alerts.sweep()
